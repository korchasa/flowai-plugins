...
/**
 * status hook: SessionStart probe.
 *
...
export function formatStatus(s: Status): string {
  ...
  if (s.lastAuditDate) {
    ...
    lines.push(
      `[memex] No audit recorded yet — consider running audit.`,
    );
  }
  if (s.uncompiled >= 5) {
    lines.push(
      `[memex] ${s.uncompiled} uncompiled raw sources (no memex page references them). Consider re-running save on them.`,
    );
  }
  lines.push(
    `[memex] Skills: save, ask, audit.`,
  );
  return lines.join("\n");
}
