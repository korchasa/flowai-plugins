...
- **Anchor** — `[ANC:mx-<type>:<slug>]` — declared once per page, ON the H1 title line, after the title text: `# Concept Name [ANC:mx-concept:concept-name]`. `<type>` matches the page's frontmatter `type:` field (`concept`, `person`, `source` for source-summary pages, `answer` for answers). `<slug>` MUST match the filename (without `.md`).
...
4. Extract key entities (people, concepts, events). For each:
   ...
   - No page → create using the appropriate entity template; declare the page's `[ANC:mx-<type>:<slug>]` on the H1 title line, after the title text.
...
6. File the answer to `pages/answers/<question-slug>.md` (always, no prompt). Declare the page's `[ANC:mx-answer:<slug>]` on the H1 title line, after the title text.
