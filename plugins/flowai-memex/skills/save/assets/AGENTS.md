...
- `ask <question>` — read `pages/index.md` → open relevant pages → follow one level of `[[wikilinks]]` → synthesize answer with citations → file to `pages/answers/<slug>.md` → optionally promote to `pages/<slug>.md`.
...
1. Run the deterministic check (`audit.ts` script) over `pages/`.
