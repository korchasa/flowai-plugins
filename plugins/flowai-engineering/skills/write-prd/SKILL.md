...
name: write-prd
description: >-
  Produce a comprehensive Product Requirements Document (PRD) for a product
  feature. Use when the user asks to write a PRD or formalize a product
  feature's scope, goals, user value, and success metrics. Not for internal
  technical/engineering change proposals (those argue for a code/architecture
  change, not a product feature).
---
