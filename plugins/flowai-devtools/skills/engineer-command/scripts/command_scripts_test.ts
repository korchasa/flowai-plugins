...
Deno.test("init: creates directory with SKILL.md and resource dirs", async () => {
  await withTempDir(async (parentDir) => {
    const result = initCommand("new-cmd", parentDir);
    assertEquals(typeof result, "string");
    assertEquals(result !== null, true);

    const cmdDir = join(parentDir, "new-cmd");
...
Deno.test("init: fails if directory already exists", async () => {
  await withTempDir(async (parentDir) => {
    ...
    await Deno.mkdir(join(parentDir, "existing"), { recursive: true });

    const result = initCommand("existing", parentDir);
