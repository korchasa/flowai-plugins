 ...
 *
 * Analyzes the project directory: detects stack, checks which init
...
async function analyzeProject(rootDir: string): Promise<AnalysisResult> {
  ...
  if (legacyLayoutDetected) {
    checks.push({
      ok: true,
      message:
        "Legacy three-file AGENTS.md layout detected — run /update to collapse.",
    });
  }
