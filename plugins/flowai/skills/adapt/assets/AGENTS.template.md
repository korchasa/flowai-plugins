...
- **Variant Analysis**: A format for comparing alternatives of any kind — approaches, designs, tools, libraries, vendors, data models, configurations. The candidates are the labeled options of the single question that asks you to choose, each option carrying its own Pros / Cons / Risks / Best-for; the trade-offs across options (e.g. speed vs. correctness, cost vs. flexibility) follow the option list inside the same question. Never describe the candidates a second time outside the question — one description, in one place. One option is acceptable when the path is clear; when it is non-obvious, surface multiple distinct options. **Each property gets its own labelled line** — `**Pros:**`, `**Cons:**`, `**Risks:**`, `**Best for:**` as four separate bullets under the option title, and the trade-offs as their own block after the option list. An option written as one paragraph is a defect, and so is an option squeezed into a single line with the properties separated by commas, semicolons or dashes: the reader compares options property by property, and a property that is missing must show as a missing line. **This binds on every option set in a reply, not only the first.** A reply usually opens more than one choice — the main one, then a failure policy, a naming decision, a rollout order. Each of those carries the same four labelled lines per option plus its own trade-offs; a correctly laid-out first question followed by bare one-line options below it fails the rule.
